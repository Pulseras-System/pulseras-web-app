...
const MainLayout = () => {
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-pink-50 to-blue-50">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md border-b border-pink-200 shadow-sm bg-pink-500/90 text-white">
        <div className="container flex items-center justify-between h-20 px-4 mx-auto">
            ...
            <NavigationMenuList className="gap-6 text-base">
              {[
                ...
                <NavigationMenuItem key={item.to}>
                  <Link
                    to={item.to}
                    className="px-4 py-2 rounded-lg transition duration-300 hover:bg-pink-500/50 hover:scale-105 font-medium"
                  >
                    {item.label}
                  </Link>
                </NavigationMenuItem>
          ...
          <div className="flex items-center gap-3">
            <ThemeToggle />
            <Button variant="outline" asChild className="border-white text-white hover:bg-pink-500 hover:text-white hover:border-pink-500">
              <Link to="/login">Đăng nhập</Link>
            </Button>
            <Button asChild className="bg-white text-pink-600 hover:bg-pink-100 hover:text-pink-700">
      ...
      {/* Footer */}
      <footer className="border-t border-pink-200 py-6 backdrop-blur-sm bg-pink-600/80 text-white">
