...
const StaffLayout = () => {
  ...
  return (
    <div className="flex min-h-screen bg-[#fff8f0]">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:block w-64 bg-gradient-to-b from-pink-800 to-pink-700 text-pink-50 flex flex-col p-6 space-y-6 fixed h-full border-r border-pink-900/10 shadow-xl">
            ...
            <Link
              ...
              className={cn(
                ...
                location.pathname.startsWith(path)
                  ? "bg-pink-600 shadow-inner font-semibold text-white"
                  : "hover:bg-pink-600/40 text-pink-100"
              )}
            >
              <Icon className="w-5 h-5 group-hover:scale-110 transition-transform" />
              <span>{label}</span>
              {location.pathname.startsWith(path) && (
                <div className="ml-auto w-2 h-2 rounded-full bg-pink-100 animate-ping" />
              )}
            </Link>
          ))}
        </nav>

        <div className="mt-auto pt-4 border-t border-pink-700/50">
          <div className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-pink-600/40 cursor-pointer transition">
            <svg 
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50">
        <div className="bg-white/80 backdrop-blur-md border-b border-pink-200 shadow-sm flex items-center justify-between p-4">
          <Sheet>
            ...
            <SheetContent side="left" className="w-[300px] bg-gradient-to-b from-pink-800 to-pink-700 text-pink-50">
              <div className="flex flex-col h-full">
                ...
                <div className="pt-4 border-t border-pink-700/50">
                  <div className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-pink-600/40 cursor-pointer transition">
                    <svg 
                      xmlns="http://www.w3.org/2000/svg" 
                      className="h-5 w-5" 
      ...
      <div className="flex-1 flex flex-col lg:ml-64 mt-16 lg:mt-0">
        ...
        <header className="hidden lg:flex h-16 px-8 items-center justify-between bg-white/80 backdrop-blur-md border-b border-pink-200 shadow-sm z-10">
          ...
          <div className="flex items-center gap-4">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <div className="flex items-center gap-2 bg-pink-100 rounded-full px-3 py-1 border border-pink-200 shadow-inner cursor-pointer">
                  <Avatar className="w-8 h-8">
                    <AvatarImage src="https://i.pravatar.cc/40?img=12" alt="Staff avatar" />
                    <AvatarFallback>NV</AvatarFallback>
                  </Avatar>
        ...
        <main className="flex-1 bg-[#fffaf3] p-8 overflow-y-auto">
          <div className="max-w mx-auto bg-white rounded-xl shadow-md p-6 border border-pink-100">
            <Outlet />
          </div>
        </main>
      </div>
