...
const BraceletCard = ({ name, description, price, imageSrc }: BraceletCardProps) => (
  <div className="group relative flex flex-col w-full overflow-hidden rounded-lg shadow-md hover:shadow-xl transition-all duration-300 border border-pink-100">
    <img
      loading="lazy"
      src={imageSrc}
      alt={`Vòng tay ${name}`}
    ...
    />
    <div className="absolute inset-0 bg-gradient-to-t from-pink-900/70 via-pink-800/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
      <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
        <h3 className="text-xl font-bold text-white">{name}</h3>
        <p className="text-pink-100 mt-1">{description}</p>
...
const HomePage = () => {
  ...
  const categories = [
    { name: "Tất cả vòng tay", href: "#all", bg: "bg-gradient-to-br from-pink-400 to-pink-600" },
  ...
  return (
    <div className="flex flex-col w-full min-h-screen bg-pink-50">
      ...
      <div className="relative flex items-center justify-center w-full min-h-[600px] overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-pink-900/60" />
        <div className="relative z-10 px-6 text-center max-w-4xl">
          ...
          <p className="mt-6 text-xl text-pink-100 drop-shadow-md">
      ...
      <section className="px-6 py-12 bg-white">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl font-bold text-center text-pink-900 mb-8">Khám phá danh mục</h2>
      ...
      {/* Sản phẩm nổi bật */}
      <section className="px-6 py-16 max-w-7xl mx-auto w-full bg-pink-50">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-pink-900">Bộ sưu tập nổi bật</h2>
          <p className="mt-2 text-pink-700">Những thiết kế được yêu thích nhất mùa này</p>
        </div>
      ...
      <section className="px-6 py-16 bg-pink-900 text-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <Button 
            variant="outline" 
            className="mt-8 border-pink-300 text-pink-300 hover:bg-pink-300/10 hover:text-pink-200"
          >
            Khám Phá Quy Trình
          </Button>
        </div>
      ...
      <section className="px-6 py-16 bg-white">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-pink-900 mb-4">Tham Gia Cộng Đồng</h2>
          <p className="text-pink-700 mb-6">Nhận ưu đãi đặc biệt và cập nhật bộ sưu tập mới nhất</p>
          <div className="flex flex-col sm:flex-row gap-2 justify-center">
            <input 
              type="email" 
              placeholder="Địa chỉ email của bạn" 
              className="px-4 py-3 rounded-lg border border-pink-300 focus:outline-none focus:ring-2 focus:ring-pink-500 flex-grow max-w-md"
            />
