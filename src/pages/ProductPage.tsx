...
const categories = [
  { name: "Tất cả vòng tay", value: "Tất cả", bg: "bg-gradient-to-br from-sky-400 to-sky-600", icon: "✨" },
...
const BraceletCard = ({
  ...
  <Link to={`/shop/${id}`} className="group block transition-transform hover:-translate-y-1">
    <div className="relative flex flex-col w-full overflow-hidden rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 border border-sky-100 bg-white">
      <div className="relative overflow-hidden aspect-square">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-sky-900/70 via-sky-800/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-5">
          <div className="transform translate-y-6 group-hover:translate-y-0 transition-transform duration-300">
            ...
            <p className="text-sky-100 mt-1 text-sm">{description}</p>
            <div className="flex justify-between items-center mt-4">
              <span className="text-lg font-semibold text-sky-200">{price}</span>
              <Button
                variant="default"
                size="sm"
                className="bg-sky-500 hover:bg-sky-600 text-white shadow-md hover:shadow-lg transition-all"
              >
                Xem chi tiết
              </Button>
            </div>
      ...
      <div className="p-4">
        <h3 className="font-medium text-sky-900 truncate">{name}</h3>
        <p className="text-sky-600 text-sm mt-1">{description}</p>
        <p className="text-sky-500 font-medium mt-2">{price}</p>
      </div>
    </div>
  </Link>
);
...
const ProductPage = () => {
  ...
  return (
    <div className="px-4 sm:px-6 py-8 bg-gradient-to-b from-sky-50 to-white min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <nav className="flex mb-6" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-1 md:space-x-2">
            <li>
              <button 
                onClick={() => navigate('/')}
                className="inline-flex items-center text-sm font-medium text-sky-700 hover:text-sky-600 hover:underline transition-colors"
              >
                Trang chủ
              </button>
            </li>
        ...
        {/* Search and Filter */}
        <div className="mb-12 bg-white p-6 rounded-xl shadow-sm border border-sky-100">
          <div className="max-w-md mx-auto mb-6 relative">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3 py-3 border border-sky-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-sky-400"
            />
          </div>

          {/* Categories */}
          <div>
            <h2 className="text-2xl font-bold text-center text-sky-900 mb-6 relative inline-block">
              <span className="relative z-10 px-4 bg-white">Danh mục vòng tay</span>
              <span className="absolute bottom-3 left-0 right-0 h-1 bg-sky-100 z-0"></span>
            </h2>
        ...
        {/* Products */}
        <div className="mb-16">
          <h2 className="text-3xl font-bold text-center text-sky-900 mb-12 relative inline-block">
            <span className="relative z-10 px-4 bg-gradient-to-b from-sky-50 to-white">
              {selectedCategory === "Tất cả" ? "Tất cả sản phẩm" : `Vòng tay ${selectedCategory}`}
            </span>
            <span className="absolute bottom-4 left-0 right-0 h-1 bg-sky-100 z-0"></span>
          </h2>
          ...
          {paginated.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-sky-100">
              ...
              <h3 className="text-xl font-medium text-sky-900 mb-2">Không tìm thấy sản phẩm</h3>
              <p className="text-sky-600 mb-6">Không có sản phẩm nào phù hợp với tiêu chí tìm kiếm của bạn</p>
              <Button
                variant="outline"
          ...
          {totalPages > 1 && (
            <div className="mt-16 flex flex-col items-center">
              <div className="flex items-center gap-2 mb-4">
                ...
                >
                  <ChevronFirst className="h-4 w-4 text-sky-600" />
                </Button>
                <Button
                  variant="outline"
                  size="icon"
                    ...
                    <Button
                      ...
                      className={`min-w-[40px] ${
                        pageNum === currentPage
                          ...
                          : "border-sky-300 hover:bg-sky-50 text-sky-600"
                      }`}
                    >
                      {pageNum}
                    </Button>
                ...
                <Button
                  ...
                  className="border-sky-300 hover:bg-sky-50"
                >
                  <ChevronLast className="h-4 w-4 text-sky-600" />
                </Button>
              </div>
