  ...
  return (
    <div className="p-6 w-full space-y-6">
      <div className="flex flex-col md:flex-row justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-sky-900">Quản lý Vòng tay</h2>
          <p className="text-sm text-sky-700">Danh sách sản phẩm trong workshop</p>
        </div>
      ...
      {showFilters && (
        <div className="bg-sky-50 p-4 rounded-lg border border-sky-200 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-sky-800 mb-1">Lọc theo giá</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-sky-800 mb-1">Lọc theo chất liệu</label>
              <select
                className="w-full p-2 border border-sky-200 rounded-md bg-white text-sky-900 focus:ring-sky-300"
      ...
      <div className="rounded-lg border border-sky-200 bg-white shadow-sm overflow-x-auto">
        <Table className="min-w-[700px]">
          <TableHeader className="bg-sky-50">
            <TableRow>
              <TableHead className="text-sky-900 w-20">Hình ảnh</TableHead>
              <TableHead className="text-sky-900">Tên sản phẩm</TableHead>
              <TableHead className="text-sky-900">Chất liệu</TableHead>
              <TableHead className="text-sky-900">Giá (VND)</TableHead>
              <TableHead className="text-sky-900">Tồn kho</TableHead>
              <TableHead className="text-sky-900 text-right">Thao tác</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {currentBracelets.map((bracelet) => (
              <TableRow key={bracelet.id} className="hover:bg-sky-50/50">
                <TableCell>
                ...
                <TableCell className="font-medium text-sky-900 whitespace-nowrap">
                  <div className="font-semibold">{bracelet.name}</div>
                  <div className="text-xs text-sky-500">ID: {bracelet.id}</div>
                </TableCell>
      ...
      {filteredBracelets.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 text-center border border-sky-200 rounded-lg bg-sky-50">
          ...
          <h3 className="text-lg font-medium text-sky-900">Không tìm thấy vòng tay</h3>
